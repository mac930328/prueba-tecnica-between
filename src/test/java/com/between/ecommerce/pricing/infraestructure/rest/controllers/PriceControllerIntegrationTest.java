...
class PriceControllerIntegrationTest {
	...
	@ParameterizedTest
	@CsvSource({"2020-06-14T10:00:00, 35455, 1, 35.50, 2020-06-14T00:00:00, 2020-12-31T23:59:59",
	...
	void testPriceRequest(String applicationDate, int productId, int brandId, double expectedPrice, String startDate,
			String endDate) throws Exception {
        this.mockMvc.perform(get("/api/price").param("applicationDate", applicationDate)
				.param("productId", String.valueOf(productId)).param("brandId", String.valueOf(brandId)))
				.andExpect(status().isOk()).andExpect(jsonPath("$.price").value(expectedPrice))
	...
	void testNotFoundPrice() throws Exception {
        ...
        this.mockMvc.perform(get("/api/price").param("applicationDate", date.toString()).param("productId", "35455")
				.param("brandId", "2")).andExpect(status().isNotFound());
	}

	@ParameterizedTest
	@MethodSource("getInvalidTestData")
	void testInvalidParameters(String applicationDate, String productId, String brandId, int httpStatus, String message)
			throws Exception {
        this.mockMvc.perform(get("/api/price").param("applicationDate", applicationDate).param("productId", productId)
				.param("brandId", brandId)).andExpect(status().is(httpStatus))
	...
	static Stream<Arguments> getInvalidTestData() {
		...
		final int httpStatusValue = HttpStatus.BAD_REQUEST.value();
		return Stream.<Arguments>builder()
				...
				.add(Arguments.of(date, "-35455", "HOLA", httpStatusValue,
						"Parameter 'brandId' should be of type Long. Provided value: HOLA"))
				.add(Arguments.of(date, "HOLA", "1", httpStatusValue,
						"Parameter 'productId' should be of type Long. Provided value: HOLA"))
				.build();
	}
